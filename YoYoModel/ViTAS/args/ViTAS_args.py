...
def config_ViTAS_args(ViTAS_dic):   
    ...
    # parser.add_argument('--VFM_type', type=str, default = 'DINOv2', choices=['DINOv2','DepthAny','BEiT3','DepthAny2'],help='args for load VFM')
    parser.add_argument('--VFM_type', type=str, default = 'DINOv2', choices=['DINOv2','DepthAny','DepthAny2'],help='args for load VFM') # 'BEiT2'
    ...
    parser.add_argument('--block_expansion', default = 0, type=int, help='if use block expansion')
    parser.add_argument('--featup_SDM', default = False, type=bool, help='if use featup for SDM')
...
def config_ViTASPSM_args(ViTAS_dic): 
    args = config_ViTAS_args(ViTAS_dic)  
    args.pre_channels = [64,128,256,256]
...
ViTAS_config = {'DINOv2':{'vit_l': {'hook_select':[5,11,17,23],'channel':1024,'hook_check':23},
                # 'vit_l_16': {'hook_select':[5,11,17,23],'channel':1024,'hook_check':23},
                'vit_b': {'hook_select':[2,5,8,11],'channel':768,'hook_check':11},},
                # 'BEiT2':{'vit_l': {'hook_select':[5,11,17,23],'channel':1024,'hook_check':23},},
                'DepthAny':{'vit_l': {'hook_select':[5,11,17,23],'channel':1024,'hook_check':23},},
                'DepthAny2':{'vit_l': {'hook_select':[5,11,17,23],'channel':1024,'hook_check':23},},
                # 'BEiT3':{'vit_l': {'hook_select':[5,11,17,23],'channel':1024,'hook_check':23},},
                }

def ViTAS_hooks_init(args): # select the init dimer hooks
...
def check_VFM_hooks(args,block_expansion): # check if the hooks is valid
    assert args.ViTAS_model in ViTAS_config[args.VFM_type].keys()
    assert args.ViTAS_hooks[-1] <= (ViTAS_config[args.VFM_type][args.ViTAS_model]['hook_check'] + block_expansion)

def args_refine(args,VFM_type,ViTAS_model,ViTAS_hooks,ViTAS_unfreeze,wo_fuse,ViTAS_fuse,ViTAS_fuse_patch,ViTAS_fuse_weight,CA_layers,block_expansion,featup_SDM):
    ...
    if not block_expansion is None:
        args.block_expansion = block_expansion 
        args.ViTAS_hooks = [hook + round(i*block_expansion/len(args.ViTAS_hooks)) for i,hook in enumerate(args.ViTAS_hooks)]
        args.ViTAS_hooks[-1] = ViTAS_config[args.VFM_type][args.ViTAS_model]['hook_check']+block_expansion
    if not featup_SDM is None:
        args.featup_SDM = featup_SDM
    assert args.ViTAS_fuse in ['SDFA','PAFM','VFM']
    assert args.ViTAS_unfreeze in ['0','1','2','3','4','5'], str(args.ViTAS_unfreeze)
    check_VFM_hooks(args,args.block_expansion)
    return args