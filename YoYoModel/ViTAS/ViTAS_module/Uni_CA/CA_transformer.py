...
from YoYoModel.ViTAS.ViTAS_module.Uni_CA.attention import (single_head_full_attention, single_head_split_window_attention,
                        single_head_full_attention_1d, single_head_split_window_attention_1d)
from YoYoModel.ViTAS.ViTAS_module.Uni_CA.CA_utils import generate_shift_window_attn_mask, generate_shift_window_attn_mask_1d
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, source, target,
        ...
        if not query.shape == key.shape:
            ...
            is_self_attn = (query - key).abs().max() < 1e-5
        ...
        if attn_type == 'swin' and attn_num_splits > 1:  # self, cross-attn: both swin 2d
            ...
            if self.nhead > 1:
                ...
                if is_self_attn:
                    ...
                    if attn_num_splits > 1:
                        if shifted_window_attn_mask_1d is None:
                            print(source.shape)
                            print(target.shape)
                            print(height,width)
                            print(attn_type)
                            print(attn_num_splits)  
                            print(is_self_attn)
                            print((query - key).abs().max())  
                            print((query - key).abs().mean())      
