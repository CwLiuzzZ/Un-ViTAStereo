...
class Feature(SubModule):
    ...
    def DDFM(self, x):
        feature_list = []
        x = self.act1(self.bn1(self.conv_stem(x))) # 1/2
        x2 = self.block0(x) # 1/2
        feature_list.append(x2)
        x4 = self.block1(x2) # 1/4
        feature_list.append(x4)
        x8 = self.block2(x4) # 1/8
        feature_list.append(x8)
        x16 = self.block3(x8) # 1/16
        x32 = self.block4(x16) # 1/32
        x16 = self.deconv32_16(x32, x16) # 1/16
        x8 = self.deconv16_8(x16, x8) # 1/8
        x4 = self.deconv8_4(x8, x4) # 1/4
